...
class WPLogin extends SingletonHook
{
	...
	public function editLoginFooter(): void
	{
		...
		$html .= sprintf(
			'<img src="%s" alt="">',
			fws()->images()->assetsSrc('login01.png')
		);

		$html .= '</div>'; // close .fws-login-illustration

		$html .= sprintf(
			'<div class="fws-footer-author"><span>' . __( '', 'Cheke`s team' ) . '',
			esc_url( 'http://fws.us/' )
		);
	...
	public function addLoginTitle($message): string
	{
		...
		if ( empty($message) ){
			$newHtml .=  '<span class="fws-login-title">' . __('Log in to your account', 'Cheke`s team') . '</span>';
		} else {
			$newHtml .= $message;
		}
